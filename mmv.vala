...
const string APPENDNAME = "mad";
const string COPYNAME = "mcp";
const string LINKNAME = "mln";
...
public enum BadStyle {
	ASK,
	SKIP,
	ABORT,
}

public enum RecaseAction {
	STAY,
	LOWER,
	UPPER,
}

const int MAXWILD = 20;
const int MAXPATLEN = Posix.PathConfName.PATH_MAX;
const int INITROOM = 10;
...
const string TEMP = "$$mmvtmp.";
const string TOOLONG = "(too long)";
const string EMPTY = "(empty)";

// FIXME: generate with reference to SLASH
const string SLASHSTR = "/";

const string PATLONG = "%.40s... : pattern too long.\n";
...
int vala_parsepat()
{
	...
	int totwilds = 0;
	bool instage = false;
	const string TRAILESC = "%s -> %s : trailing %c is superfluous.\n";

	// Parse "from"
	lastname = 0;
	if (from.substring(0, 1) == "~" + SLASHSTR) {
		if ((homelen = home.length) + fromlen > MAXPATLEN) {
			Posix.stdout.printf(PATLONG, from);
			return -1;
		}
		from = home + from.substring(1);
		lastname = homelen + 1;
	}
	nstages = 0;
	char c;
	long p;
	for (p = lastname; (c = from[p]) != '\0'; p++) {
		switch (c) {
		case SLASH:
			lastname = p + 1;
			if (instage) {
				if (firstwild_vala[nstages] == -1)
					firstwild_vala[nstages] = p;
				stager_vala[nstages++] = p;
				instage = false;
			}
			break;
		case ';':
		...
		case '[':
			if (c == ';' && lastname != p) {
				Posix.stdout.printf("%s -> %s : badly placed ;.\n", from, to);
				return -1;
			}
			if (totwilds++ == MAXWILD) {
			...
			if (instage) {
				...
				stagel_vala[nstages] = lastname;
				firstwild_vala[nstages] = (c == ';' ? -1 : p);
				nwilds[nstages] = 1;
				instage = true;
			}
			if (c != '[')
	...
	// Parse "to"
	lastname = 0;
	if (to.substring(0, 1) == "~" + SLASHSTR) {
		if ((homelen = home.length) + tolen > MAXPATLEN) {
			Posix.stdout.printf(PATLONG, to);
			return -1;
		}
		to = home + to.substring(1);
		lastname = homelen + 1;
	}

	for (p = lastname; (c = to[p]) != '\0'; p++) {
		switch (c) {
		case SLASH:
			if (FileOp.DIRMOVE in op) {
				Posix.stdout.printf("%s -> %s : no path allowed in target under -r.\n",
					   from, to);
				return -1;
			}
			lastname = p + 1;
...
void matchpat()
{
	if (parsepat() != 0)
		paterr = 1;
	else if (dostage(from, pathbuf, start, length, 0, 0) != 0) {
		Posix.stdout.printf("%s -> %s : no match.\n", from, to);
		paterr = 1;
	}
}

void domatch(string cfrom, string cto)
{
	if ((fromlen = cfrom.length) >= MAXPATLEN) {
		Posix.stdout.printf(PATLONG, cfrom);
		paterr = 1;
	} else if ((tolen = cto.length) >= MAXPATLEN) {
		Posix.stdout.printf(PATLONG, cto);
		paterr = 1;
	} else {
		from = cfrom;
		to = cto;
...
public int main(string[] args) {
	...
	string frompat, topat;
	pathbuf = new char[Posix.PathConfName.PATH_MAX];
	stagel_vala = new long[MAXWILD];
	firstwild_vala = new long[MAXWILD];
	stager_vala = new long[MAXWILD];
	for (int i = 0; i < MAXWILD; i++) {
		stagel_vala[i] = -1;
		firstwild_vala[i] = -1;
		stager_vala[i] = -1;
	}
	nwilds = new int[MAXWILD];
	start = new string[MAXWILD];
	length = new long[MAXWILD];

//	struct stat dstat;

	if ((home = Environment.get_variable("HOME")) == null || home == SLASHSTR)
		home = "";
//	if (!stat(".", &dstat)) {
//		cwdd = dstat.st_ino;
//		cwdv = dstat.st_dev;
...
//	ndirs_nonexistent = 0;
	...
	bool noex = args_info.dryrun_given != 0;
	bool matchall = args_info.hidden_given != 0;
	bool mkdirs = args_info.makedirs_given != 0;
	...
	if (args_info.move_given != 0)
		...
		if (program_name == COPYNAME)
			op = NORMCOPY;
		else if (program_name == LINKNAME)
			op = HARDLINK;
		else
			op = XMOVE;
	}
...
// #endif
	...
	if (badstyle != ASK && delstyle == ASK)
		delstyle = NO;

	if (args_info.inputs.length != 2) {
		/* Print message to stderr, not stdout. */
		print(@"not 2: $(args_info.inputs.length)");
	...
	checkcollisions();
	findorder();
	if ((op & (FileOp.COPY | FileOp.LINK)) != 0)
		nochains();
	scandeletes(baddel);
	goonordie();
	if (delstyle == DeleteStyle.ASK)
		scandeletes(skipdel);
	doreps();

	if (failed)
		return 2;
	else if (nreps == 0 && (paterr > 0 || badreps > 0))
		return 1;
	return 0;
}
